...
public class ExcelFileReader implements AutoCloseable {
	 ...
	 */
	public List<String>
	readStringsInRow(String sheetName, int row, int start)
			throws SheetNotFoundException
	{
		// do closed check, arg count/value check, sheet exists check
		...
		return readStringsInRowFromStartToEnd(rrow, start, rrow.getLastCellNum());
	}
	...
	readStringsInRowFromStartToEnd(Row row, int start, int end)
			throws SheetNotFoundException
	{
		List<String> strings = new ArrayList<String>(end - start);
			...
			throws SheetNotFoundException
	{
		...
		return new ArrayList<String>(Arrays.asList(builder.toString().split(",")));
	}
	...
	private void readFromSheet() throws IOException {
	...
	private static final String CLOSED_ERR_MSG = "workbook is null, you may be"
			+ " trying to use an ExcelFileReader instance method after calling "
			+ "its 'close()' method.";
	
	/* helper to throw a *helpful* NullPointerException if workbook is null */
	private void throwIfClosed() throws NullPointerException {
		Objects.requireNonNull(workbook, CLOSED_ERR_MSG);
	}
	
	
	private static final String NEGATIVE_ERR_MSG = "method arguments must be"
			+ " positive.";
	private static final String START_GT_END_ERR_MSG = "start index is greater"
			+ " than end index.";
	
	/* helper to throw if int method arguments are negative or invalid range */
	private void checkArgs(int index, int start, int end) throws IllegalArgumentException {
		if((index | start | end) < 0)
			throw new IllegalArgumentException(NEGATIVE_ERR_MSG);
		if(start > end)
			throw new IllegalArgumentException(START_GT_END_ERR_MSG);
	}
